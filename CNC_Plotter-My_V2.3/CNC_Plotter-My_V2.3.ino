...
// Creating object from class Screen:
Screen myScreen;
...
void setup() {
  ...
  // Screen setup:
  myScreen.begin();
  myScreen.drawLoadingAnimation(500);
  ...
  // Printing data on the screen after setup:
  myScreen.printString("Mini CNC Plotter  alive andkicking!");
  delay(2500);
...
void loop() 
{
  ...
  String comment = "";                  //String with comments 
  int posX = 0;
      ...
      if (( c == '\n') || (c == '\r') ) {             // End of line reached
        if ( lineIndex > 0 ) {                        // Line is complete. Then execute!
          ...
          if (verbose) { 
            ...
            Serial.println( line ); 
            //myScreen.printString("Recieved: ");
            //myScreen.printString(line);
          }
        ...
        if ( (lineIsComment) || (lineSemiColon) ) {   // Throw away all comment characters
          /*
          if(comment.length() < myScreen.maxString())
          {
           comment = comment + c;
           posX = posX+symbolSize;
           myScreen.printString(line[lineIndex], myScreen.getDefX()+posX, myScreen.getDefY);    
          } else {
            comment = "";
            pos = 0;
          }
...
void processIncomingLine( char* line, int charNB ) {
  ...
  while( currentIndex < charNB ) {
    ...
    case 'U':
      penUp(); 
      myScreen.printMessage("Pen UP",500);
      break;
    
    case 'D':
      myScreen.printMessage("Pen DOWN",500);
      penDown(); 
    ...
    case 'M':
      ...
      case 300:
        {
          ...
          if (Spos == 30) { 
            penDown(); 
            myScreen.printMessage("Pen DOWN",500);          // Print on the screen
          }
          if (Spos == 50) { 
            penUp(); 
            myScreen.printMessage("Pen UP",500);            // Print on the screen
          }
          break;
        }
      ...
      default:
        ...
        // Print on the screen:
        /*
        myScreen.printString("Not recognized:M");
        myScreen.printString(buffer,myScreen.getDefX() + 10*6, myScreen.getDefY()+20);
        delay(1000);
        myScreen.clear();
        */
      }
